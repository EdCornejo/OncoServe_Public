...
sys.path.append(os.path.join(dirname(dirname(realpath(__file__))),'OncoNet_Public'))
sys.path.append(os.path.join(dirname(dirname(realpath(__file__))), 'OncoData_Public'))
...
def serve():
    ...
    try:
        ...
        if onconet_args.use_risk_factors:
            ...
            risk_factors = json.loads(request.files.getlist('risk_factors')[0].read())
